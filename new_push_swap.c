...
Unit	*find_pair_from_a(Stack **new, int num)
{
	Unit *prev;
	Unit *curr;
	...
	while ((*new)->A->next)
	{
		if (prev->num < num && (*new)->A->num > num)
			return (prev);
		prev = (*new)->A;
		(*new)->A = (*new)->A->next;
	}
	if (find_max_struct(&(*new)->A)->num < num)
		return (find_min_struct(&(*new)->A));
	if (curr->num < num)
	{
		while (curr)
			curr = curr->next;
...
void find_two_pairs(Stack *new)
{
	...
	while (stack_b)
	{
		stack_a = find_pair_from_a(&new, stack_b->num);
